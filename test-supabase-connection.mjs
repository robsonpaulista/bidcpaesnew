...
const SUPABASE_ANON_KEY = process.env.SUPABASE_ANON_KEY || env.SUPABASE_ANON_KEY || process.env.VITE_SUPABASE_ANON_KEY || env.VITE_SUPABASE_ANON_KEY
...
if (SUPABASE_URL) {
  console.log(`    ${SUPABASE_URL.substring(0, 30)}...`)
}
    ...
    for (const table of tables) {
      try {
        ...
        if (response.ok) {
          ...
          results[table] = `⚠️ Erro: ${response.status} ${errorText.substring(0, 50)}`
        }
      } catch (error) {
